...
//    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType==SENDER_VIEW_TYPE){
            View view= LayoutInflater.from(context).inflate(R.layout.outgoing_bubble,parent,false);
            return new SenderViewHolder(view);
        }
    else {
            View view= LayoutInflater.from(context).inflate(R.layout.incoming_bubble,parent,false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder.getClass()==SenderViewHolder.class){
            ...
            SimpleDateFormat simpleDateFormat=new SimpleDateFormat("h:mm a");//"HH:mm:ss" is a 24 hrs format,for the am/pm format type instead h:mm a
            String strDate=simpleDateFormat.format(date);
            ((SenderViewHolder)holder).sendingTime.setText(strDate);
        }
        else {
            ...
            SimpleDateFormat simpleDateFormat=new SimpleDateFormat("h:mm a");//"HH:mm:ss" is a 24 hrs format,for the am/pm format type instead h:mm a
            String strDate=simpleDateFormat.format(date);
            ((ReceiverViewHolder)holder).receivingTime.setText(strDate);
        }
    }
    ...
    public class ReceiverViewHolder extends RecyclerView.ViewHolder{
        TextView receiverMessage, receivingTime;

        public ReceiverViewHolder(@NonNull View itemView) {
            super(itemView);
            receiverMessage=itemView.findViewById(R.id.receiverText);
            receivingTime=itemView.findViewById(R.id.receivingTime);
        }
    }
    public class SenderViewHolder extends RecyclerView.ViewHolder{
TextView senderMessage,sendingTime;
